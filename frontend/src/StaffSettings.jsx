...
export const Search = ({
  ...
  <label className="search-box search-box-v4">
    ...
    {Number.isFinite(count) && <span className="search-count">{count}</span>}
  </label>
);
  ...
  return (
    ...
    >
      ...
      >
        <header>
          <div>
            <span className="eyebrow">SCHOOLHOUSE</span>
            <h2>{title}</h2>
          </div>
          <button
            type="button"
...
export function StaffManager({ staff, roles, classes, busy, run }) {
  ...
  const q = query.toLowerCase(),
    shown = staff.filter((s) =>
      [s.name, s.email, s.staffProfile?.employeeNumber, s.staffRole?.name].some(
        (v) =>
          String(v || "")
            .toLowerCase()
            .includes(q),
      ...
      await api("/admin/staff", {
        ...
        body: {
          ...
          qualifications: d
            ...
            .map((x) => x.trim())
            .filter(Boolean),
          salaryMinor: d.get("salary")
            ? Math.round(Number(d.get("salary")) * 100)
            : null,
          payrollStatus: d.get("payrollStatus"),
          classIds: d.getAll("classIds"),
        },
      });
      setDialog(null);
  ...
  const saveRole = (e) => {
    ...
    const d = new FormData(e.currentTarget);
    run(async () => {
      await api("/admin/staff-roles/" + selected.id, {
        method: "PUT",
        body: {
          name: d.get("name"),
          description: d.get("description"),
      ...
      setDialog(null);
    }, "Role updated; affected staff must sign in again");
  };
  const saveClasses = (e) => {
    e.preventDefault();
    const d = new FormData(e.currentTarget);
    run(async () => {
      await api("/admin/staff/" + selected.id, {
        method: "PATCH",
        body: { classIds: d.getAll("classIds") },
      });
      setDialog(null);
    }, "Teacher class assignments updated");
  };
  return (
    <>
      <div className="toolbar">
        <Search
          value={query}
          onChange={setQuery}
          placeholder="Search staff, role or employee number"
        />
        <button onClick={() => setDialog("staff")}>+ Add staff</button>
        <button className="secondary" onClick={() => setDialog("role")}>
          + Create role
        </button>
      </div>
      <section className="panel table-wrap">
              ...
              <tr key={s.id}>
                <td>
                  ...
                  <small>
                    {s.email}
                    <br />
                    {s.staffProfile?.employeeNumber || "Profile pending"}
                  </small>
                </td>
                <td>{s.staffRole?.name || "Unassigned"}</td>
                <td>
                  {human(s.staffProfile?.payrollStatus || "NOT_CONFIGURED")}
                  <small>
                    {s.staffClasses?.length || 0} classes ·{" "}
                    {s.teachingAssignments?.length || 0} courses
                  </small>
                </td>
                <td>
        ...
        {!shown.length && <p className="empty">No staff match your search.</p>}
      </section>
      ...
      >
        <form className="form-grid" onSubmit={createStaff}>
          ...
          <Field title="Email / login">
            <input name="email" type="email" required />
          </Field>
          <Field title="Initial password">
            <input
              name="password"
              type="password"
              minLength={12}
          ...
          <Field title="Qualifications (comma separated)">
            <input name="qualifications" />
          </Field>
          <Field title="Monthly salary">
            <input name="salary" type="number" min="0" step=".01" />
          </Field>
          <Field title="Payroll status">
          ...
          <Field title="Staff role">
            <select name="staffRoleId" required>
      ...
      <Modal
        title={"Edit " + (selected?.name || "role")}
        open={dialog === "editRole"}
        onClose={() => setDialog(null)}
        wide
      >
        {dialog === "editRole" && selected && (
          <form onSubmit={saveRole}>
            ...
            <PermissionChecks
              selected={(selected.grants || []).map((g) => g.permission)}
            />
            <div className="modal-actions">
              <button
                type="button"
      ...
      >
        ...
        >
          ...
          <div className="modal-actions">
            ...
            <button disabled={busy}>Reset credentials</button>
          </div>
        </form>
      </Modal>
      <Modal
        title={
          "Assign class teacher duties to " + (selected?.name || "teacher")
        }
        open={dialog === "classes"}
        onClose={() => setDialog(null)}
      >
        {selected && (
          <form onSubmit={saveClasses}>
            <p className="muted">
              Selected classes will show this staff member as class teacher and
              grant class-management access.
            </p>
            {classes.map((c) => (
              <label className="check" key={c.id}>
                <input
                  ...
                  value={c.id}
                  defaultChecked={
                    selected.staffClasses?.some((x) => x.classId === c.id) ||
                    c.classTeacher?.id === selected.id
                  }
                />
                {c.name}
              </label>
            ...
            <div className="modal-actions">
              ...
              <button disabled={busy}>Save class teacher assignment</button>
            </div>
          </form>
        )}
      </Modal>
...
const cleanSettings = (s) =>
  Object.fromEntries(
    settingsKeys.map((k) => [
      k,
      s[k] ?? (Object.hasOwn(settingDefaults, k) ? settingDefaults[k] : ""),
    ]),
  );
export function SettingsPanel({ section, settings, save, busy }) {
  ...
  const submit = async (e) => {
    ...
    if (section === "Personalization") {
      ...
      for (const key of [
        ...
        const f = d.get(key);
        if (f?.size) body[key] = (await uploadFile(f, "branding")).id;
      }
    }
    if (section === "Configuration") {
      for (const k of [
  ...
  const title =
    section === "Personalization"
      ? "School identity and report branding"
    ...
    <>
      ...
      {section === "Personalization" && (
        <section className="panel branding-preview">
          <div>
            <span className="eyebrow">CURRENT BRANDING</span>
            <h2>Uploaded school assets</h2>
            <p className="muted">
              A placeholder or unavailable label means the stored image file
              must be uploaded again.
            </p>
          </div>
          <article>
            <BrandImage
              src={settings.logoPath ? "/api/assets/" + settings.logoPath : ""}
              name={settings.schoolName}
              className="branding-sample"
            />
          ...
          <article>
            {settings.watermarkPath ? (
              <OptionalImage
                src={"/api/assets/" + settings.watermarkPath}
                className="branding-sample"
                alt="Report watermark"
                fallback={<span className="branding-missing">Unavailable</span>}
              />
            ) : (
              <span className="branding-missing">Not set</span>
            )}
            <strong>Watermark</strong>
          </article>
          <article>
            {settings.principalSignaturePath ? (
              <OptionalImage
                src={"/api/files/" + settings.principalSignaturePath}
                className="branding-sample signature-sample"
        ...
        <form onSubmit={submit}>
          {section === "Personalization" && (
            <div className="form-grid">
              ...
              <Field title="School logo">
                <input name="logoPath" type="file" accept=".png,.jpg,.jpeg" />
              </Field>
              <Field title="Report watermark">
                <input
                  name="watermarkPath"
                  type="file"
                  accept=".png,.jpg,.jpeg"
              ...
              <Field title="Principal signature">
                <input
          ...
          {section === "Configuration" && (
            <>
              <div className="form-grid">
                <Field title="Academic year">
                  <input
                    name="academicYear"
                    defaultValue={settings.academicYear}
                  />
                </Field>
                <Field title="Current term label">
                  <input
                    name="currentTerm"
                ...
                <Field title="Default passing mark">
                  <input
                    name="passingMark"
                    type="number"
                    min="0"
              ...
              <label className="check">
                <input
                  ...
                  type="checkbox"
                  defaultChecked={settings.kioskFullscreen}
                />
                Require fullscreen during student exams
              </label>
              <label className="check">
                <input
                  name="syncEnabled"
...
function FeatureStatus({ features = {} }) {
  const labels = {
    ...
    reports: "Reports and promotions",
    hostel: "Hostels",
    payments: "Payments",
    communications: "Mailing and SMS",
      ...
      <div className="feature-status-grid">
        {Object.entries(labels).map(([key, label]) => (
          <article key={key}>
            <span
              className={
                features[key] === false ? "status-dot off" : "status-dot"
              }
            />
            <strong>{label}</strong>
            <small>
