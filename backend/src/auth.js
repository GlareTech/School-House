...
export const publicUser = u => ({ id:u.id, organizationId:u.organizationId, name:u.name, email:u.email, role:u.role, classId:u.classId, profilePictureId:u.profilePictureId||null, staffRole:u.staffRole ? { id:u.staffRole.id, name:u.staffRole.name } : null, permissions:u.role === 'ADMIN' ? PERMISSIONS : (u.staffRole?.grants || []).map(g => g.permission) });
export function authRoutes(app) {
  app.get('/api/auth/plans', async (_req, res) => {
    res.json(await db.subscriptionPlan.findMany({ where:{active:true}, select:{id:true,code:true,name:true,description:true,amountMinor:true,currency:true,interval:true,maxStudents:true,features:true}, orderBy:{amountMinor:'asc'} }));
  });
  app.post('/api/auth/signup', async (req, res) => {
    const input=z.object({schoolName:z.string().trim().min(2).max(120),name:z.string().trim().min(2).max(120),email:z.string().email().transform(v=>v.toLowerCase()),password:z.string().min(12).max(128),planCode:z.string().trim().min(1).max(40)}).parse(req.body);
    if(!config.PAYSTACK_SECRET_KEY)throw new HttpError(503,'Online subscription checkout is being configured');
    if(await db.user.findUnique({where:{email:input.email}}))throw new HttpError(409,'An account already exists for this email');
    const plan=await db.subscriptionPlan.findFirst({where:{code:input.planCode,active:true}});if(!plan)throw new HttpError(400,'Choose a valid subscription package');
    const reference=`schoolhouse_${Date.now()}_${randomBytes(8).toString('hex')}`;
    const passwordHash=await bcrypt.hash(input.password,12);
    await db.signupIntent.create({data:{schoolName:input.schoolName,name:input.name,email:input.email,passwordHash,reference,planId:plan.id}});
    try {
      const transaction=await paystack('/transaction/initialize',{method:'POST',body:JSON.stringify({email:input.email,amount:String(config.PAYSTACK_TOKEN_AMOUNT),currency:plan.currency,reference,channels:['card'],callback_url:`${config.APP_ORIGINS[0]}/?payment=${encodeURIComponent(reference)}`,metadata:JSON.stringify({schoolName:input.schoolName,planCode:plan.code,purpose:'trial_card_authorization'})})});
      res.status(201).json({authorizationUrl:transaction.authorization_url});
    } catch(error) { await db.signupIntent.deleteMany({where:{reference}});throw error; }
  });
  app.post('/api/auth/signup/verify', async(req,res)=>{
    const {reference}=z.object({reference:z.string().regex(/^schoolhouse_[A-Za-z0-9_]+$/)}).parse(req.body);
    const intent=await db.signupIntent.findUnique({where:{reference},include:{plan:true}});if(!intent)throw new HttpError(404,'Signup session not found');
    const transaction=await paystack(`/transaction/verify/${encodeURIComponent(reference)}`);
    const authorization=transaction.authorization;
    if(transaction.status!=='success'||transaction.amount!==config.PAYSTACK_TOKEN_AMOUNT||transaction.customer?.email?.toLowerCase()!==intent.email||authorization?.channel!=='card'||!authorization?.reusable)throw new HttpError(402,'A reusable card authorization is required to start the trial');
    let planCode=intent.plan.paystackPlanCode;
    if(!planCode){const remotePlan=await paystack('/plan',{method:'POST',body:JSON.stringify({name:`Schoolhouse ${intent.plan.name}`,amount:intent.plan.amountMinor,interval:intent.plan.interval,currency:intent.plan.currency})});planCode=remotePlan.plan_code;await db.subscriptionPlan.update({where:{id:intent.planId},data:{paystackPlanCode:planCode}});}
    const trialStartsAt=new Date(),trialEndsAt=new Date(Date.now()+7*86400000);
    const remoteSubscription=await paystack('/subscription',{method:'POST',body:JSON.stringify({customer:transaction.customer.customer_code,plan:planCode,authorization:authorization.authorization_code,start_date:trialEndsAt.toISOString()})});
    const slugBase=intent.schoolName.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,45)||'school';
    const result=await db.$transaction(async tx=>{
      const organization=await tx.organization.create({data:{name:intent.schoolName,slug:`${slugBase}-${randomBytes(3).toString('hex')}`,trialEndsAt}});
      const user=await tx.user.create({data:{organizationId:organization.id,email:intent.email,name:intent.name,passwordHash:intent.passwordHash,role:'ADMIN'}});
      await tx.appSetting.create({data:{organizationId:organization.id,schoolName:intent.schoolName,gradingScale:[]}});
      await tx.subscription.create({data:{organizationId:organization.id,planId:intent.planId,status:'TRIALING',trialStartsAt,trialEndsAt,nextChargeAt:trialEndsAt,paystackCustomerCode:transaction.customer.customer_code,paystackSubscriptionCode:remoteSubscription.subscription_code,paystackEmailToken:remoteSubscription.email_token,authorizationCode:authorization.authorization_code,authorizationSignature:authorization.signature,cardBrand:authorization.card_type,cardLast4:authorization.last4}});
      await tx.signupIntent.delete({where:{id:intent.id}});return {user,organization};
    });
    sendWelcomeEmail({email:intent.email,name:intent.name,schoolName:intent.schoolName,trialEndsAt,planName:intent.plan.name}).catch(()=>{});
