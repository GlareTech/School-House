...
export function adminRouter() {
  ...
  r.post('/staff/import',admin,async(req,res)=>{
    const {rows}=importRows.parse(req.body),errors=[];let imported=0;
    for(let i=0;i<rows.length;i++)try{const row=rows[i],role=await db.staffRole.findUnique({where:{name:String(row.roleName||'').trim()}});if(!role)throw new Error('Unknown role');const classNames=String(row.classNames||'').split('|').map(x=>x.trim()).filter(Boolean),classes=classNames.length?await db.class.findMany({where:{name:{in:classNames}}}):[];if(classes.length!==classNames.length)throw new Error('Unknown class');const password=passwordSchema.parse(String(row.password||''));await db.user.create({data:{name:String(row.name||'').trim(),email:z.string().email().parse(String(row.email||'').toLowerCase()),passwordHash:await bcrypt.hash(password,12),role:'STAFF',staffRoleId:role.id,staffProfile:{create:{employeeNumber:String(row.employeeNumber||'').trim(),phone:String(row.phone||''),address:String(row.address||''),qualifications:String(row.qualifications||'').split('|').filter(Boolean)}},staffClasses:{create:classes.map(c=>({classId:c.id}))}}});imported++;}catch(e){errors.push({row:i+2,error:e.message})}
